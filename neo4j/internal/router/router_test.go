...
func TestMultithreading(t *testing.T) {
	...
	consumer := func() {
		for i := 0; i < 30; i++ {
			readers, err := router.GetOrUpdateReaders(context.Background(), nilBookmarks, dbName, nil, nil, false)
			if len(readers) != 2 {
			...
			writers, err := router.GetOrUpdateWriters(context.Background(), nilBookmarks, dbName, nil, nil, false)
			if len(writers) != 1 {
...
func TestRespectsTimeToLiveAndInvalidate(t *testing.T) {
	...
	if _, err := router.GetOrUpdateReaders(ctx, nilBookmarks, dbName, nil, nil, false); err != nil {
...
func TestUsesRootRouterWhenPreviousRoutersFails(t *testing.T) {
	...
	if _, err := router.GetOrUpdateReaders(context.Background(), nilBookmarks, dbName, nil, nil, false); err != nil {
		testutil.AssertNoError(t, err)
	}
...
func TestUseGetRoutersHookWhenInitialRouterFails(t *testing.T) {
	...
	_, err := router.GetOrUpdateReaders(context.Background(), nilBookmarks, dbName, nil, nil, false)
...
func TestCleanUp(t *testing.T) {
	...
	ctx := context.Background()
	if _, err := router.GetOrUpdateReaders(ctx, nilBookmarks, "db1", nil, nil, false); err != nil {
		testutil.AssertNoError(t, err)
	}
	if _, err := router.GetOrUpdateReaders(ctx, nilBookmarks, "db2", nil, nil, false); err != nil {
		testutil.AssertNoError(t, err)
	}
